-- 🚀 Schema Avançado n.Gabi + Supabase
-- Inclui todas as funcionalidades: Auth, RLS, Realtime, Storage, Functions
...
-- Função para calcular uso de tokens
CREATE OR REPLACE FUNCTION calculate_token_usage(p_text TEXT)
RETURNS INTEGER AS $$
BEGIN
    -- Implementação simples: ~4 caracteres por token
    RETURN LENGTH(p_text) / 4;
END;
$$ LANGUAGE plpgsql;
...
GROUP BY DATE_TRUNC('hour', ch.created_at)
ORDER BY hour DESC;
...
-- Criar buckets de storage (executar via SQL Editor)
...
SELECT '✅ Schema Supabase n.Gabi configurado com sucesso!' as status; 